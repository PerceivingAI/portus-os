...
The same verified `artools 0.39.1-1` seam also requires `basestrap -c` host-cache sharing and the selected `-C "${pacman_conf}"` path. PortusOS uses those properties to close rolling-repository state before construction. After mounting the run-owned Artix context, the helper performs a fresh stable-repository sync, copies and freezes the captured `system`/`world`/`galaxy` databases in a separate immutable snapshot, resolves the complete profile graph against an empty run-owned database, and acquires/SHA-256 verifies the exact archives into the persistent Artix package cache. The eventual `file://` repository is not created until every frozen identity is present and the package-progress ledger reports complete verification with zero pending files. Only then are the frozen DB snapshot and verified archives exposed as the run-local repository, independently re-resolved with network repositories disabled, and the run-owned stable artools pacman config switched to that local `file://` source. Cache ownership is restored and the verified cache is remounted read-only inside the private namespace before `buildiso`. Canonical run `20260829T072729Z-4164361b115a-dev-first-live` proved the fresh sync and 667-package resolution but failed in the acquisition phase before local-repository validation or `buildiso`: a single roughly 1.47-GiB `pacman -Sw` transaction encountered repeated low-speed timeouts and TLS EOFs across several mirrors. A2 now keeps that graph immutable while replacing the network prefetch with deterministic 192 MiB byte-bounded batches of exact `repository/package` targets against the A1 anchor. Pacman dependency expansion is disabled for those download-only transactions, every batch is SHA-256 verified immediately, only verified files leave the pending set, and aggregate batch/pending progress is persisted after each attempt. A single archive larger than the ceiling is isolated in its own batch. A3 now adds controlled package-mirror retry/failover: each batch uses the A1 anchor first, then at most three distinct subsequent HTTPS entries after that anchor in official mirrorlist order, while continuing to read the same frozen DB. Pacman 7.1 was verified locally: `-Sw` is download-only and doubled `--nodeps` skips all dependency checks, so the adapter can request exact already-resolved `repository/package` identities without provider/dependency re-resolution. Acquisition never uses `-y`/`-Sy`/`-Syy`; SHA-valid completed files are retained, corrupt or partial payload is removed before switching mirrors, and a nonzero pacman result cannot become success from hash evidence alone. If that nonzero transaction leaves all requested archives SHA-valid, detached `.sig` files are removed before the mandatory clean online retry so pacman can reacquire authoritative signature material on the next mirror. A4 now adds explicit cross-run cache reconciliation before batching: after the new run freezes its package graph, each expected persistent-cache archive is re-hashed against that graph; exact matches become reused, mismatched completed archives are removed, stale `.part`/`.sig.part` payload and orphan detached signatures are removed, and only pending identities enter A2/A3. The frozen package identity also records a hash of repository-provided PGP-signature metadata; when that metadata already carries the signature, a detached `.sig` beside a reused exact-hash archive is redundant and is removed before local-only validation. The currently retained failed-run cache has 13 complete candidate archives and no `.part` files, but each remains conditional on matching the next frozen SHA rather than being trusted as historical state. Passing evidence must show a complete reused/pending partition and consistent final `cached_before` annotations. A5 now persists one current package state (`pending`, `reused_verified`, `downloaded_verified`, `corrupt_removed`, or `failed`) for every frozen identity, with size/verification/acquisition state and last batch/attempt/mirror/failure class; aggregate resolved/verified/pending package and byte totals are refreshed after cache audit and every mirror attempt. Attempts also carry requested/verified/pending filename and byte sets, result and normalized failure class, so failed closure evidence retains real partial progress. Cache-level verified/pending/downloaded counters are updated in the same write, and current failed evidence is accepted only when the outer ledger can reconstruct the attempt partitions and bind each attempted package's final state to its latest batch/attempt/mirror/failure class. A6 now adds structured substage/cause diagnosis across mirror selection, repository synchronization, resolution, acquisition, cache verification and local validation; acquisition stderr is replayed while captured for machine classification. Only the terminal failed anchor/mirror attempt may contribute attempt context, and current acquisition cause/context must agree with the terminal A5 failure class, so recovered network failures cannot be recycled as the explanation for a later unrelated error. Explicit HTTP evidence is required for a 404 classification rather than generic `target not found` text. The outer run keeps stage `repository-closure` while surfacing the validated diagnosis in its reason, including cause `interrupted` when SIGINT terminates an active closure; interrupts after closure has passed remain `native-iso-build`. Historical string-only failure records remain readable. A7 makes the final handoff executable and testable: the synchronized DBs and separate DB snapshot are hard-frozen as read-only bind mounts; `repository-closure.json` proves the local repository was created only after complete package verification and then frozen read-only before matching local-only resolution. The locked stable artools pacman config is rewritten to the exact local `file://` source, hash-bound and frozen read-only before the local whole-graph pacman check. After that check, every archive is SHA-256 reverified, cache ownership is restored, and both the cache source and chroot bind become read-only. The native process launcher then performs a live pre-exec check of the pacman-config hash, package/DB hashes, exact repository symlinks and mount state instead of trusting evidence flags alone, while the outer ledger rejects PASS evidence without the full gate. Regression tests prove unresolved packages, post-validation cache/DB drift and lost read-only mount state all keep the buildiso runner unreachable. The later whole-graph `-Sw` is local-only validation after the cache is complete, not a network prefetch. Only after every archive is verified may subsequent rootfs/livefs/bootfs basestrap transactions see the frozen local repository.
