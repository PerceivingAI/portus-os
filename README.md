...
**Current build frontier (2026-08-29):** native Artix construction is implemented, and the repository/package-closure defect exposed by run `20260829T063320Z-658f8230fa32-dev-first-live` is now hardened in the supported native path. Before `buildiso`, the run-owned Artix context refreshes the stable `system`/`world`/`galaxy` databases, records their hashes, resolves the exact transitive package closure from an empty per-run pacman database, verifies or reuses/prefetches every package by the repository SHA-256, and validates the same closure through a run-local repository snapshot. `buildiso` then receives a local-only pacman configuration and a verified package cache remounted read-only inside the private native namespace; reusable cache ownership is returned to the unprivileged build owner. The run emits checksum-bound `repository-closure.json` evidence, and preflight now treats the closure implementation as PASS/BLOCK rather than the former warning. Host-safe regression tests cover repository identity drift plus missing/corrupt cache entries. This implementation still requires one real canonical native run to prove the frozen closure under `buildiso` and to re-test the tracked `memtest86+` bootfs correction through squashfs/final ISO generation. No candidate ISO has been produced or accepted yet.
