...
**Current build frontier (2026-08-29):** native Artix construction is implemented and the repository-closure gate has now been exercised by canonical run `20260829T072729Z-4164361b115a-dev-first-live`. That run passed preflight, tests, staging and the owner-authorized sudo handoff; freshly synchronized the stable `system`/`world`/`galaxy` databases; recorded their hashes; and resolved an exact 667-package closure totaling about 1.47 GiB of downloads. It then failed **before `buildiso`** during the initial bulk `pacman -Sw` prefetch because multiple Artix mirrors were too slow or terminated TLS transfers. The gate correctly emitted `repository-closure.json` with `status: fail`, classified the run as `repository-closure` failure, prevented construction from starting with an incomplete package universe, and cleanup again passed. This proves the stale-prepared-database defect has been moved out of `buildiso`. A0 now persists the complete resolved graph to `repository-closure.json` before package acquisition, so later failures cannot collapse the evidence back to `packages: []`. A1 now also selects one healthy active HTTPS entry from the official Artix mirrorlist and uses that same anchor to synchronize `system`, `world` and `galaxy` into the empty per-run database; the selected mirror, attempt results, generated config hash and DB hashes are recorded, then the per-run sync DB is made read-only before resolution. A2 now replaces the monolithic network prefetch with deterministic 192 MiB byte-bounded batches of exact `repository/package` identities against the anchored frozen DB; pacman runs with dependency expansion disabled, each batch is SHA-256 verified immediately, verified files are removed from the pending set, and aggregate batch/pending counts are persisted after every attempt. A3 now adds controlled package-mirror failover: every batch tries the A1 anchor first and then at most three distinct active HTTPS fallbacks in official mirrorlist order, always against the same frozen read-only repository DB and exact `repository/package` identities. Each attempt uses pacman 7.1 download-only `-Sw` with doubled `--nodeps`, never refreshes repository metadata, preserves SHA-valid completed files, removes corrupt/incomplete files before fallback, and requires a clean pacman transaction before a batch can pass. A4 now makes that persistent cache explicitly resumable across runs: after each new frozen closure is resolved, the helper audits every exact filename against the new repository-provided SHA-256, records matching complete archives as `reused`, removes mismatched completed archives and associated stale signatures, removes stale `.part`/`.sig.part` payload, and sends only audited `pending` identities into A2 batching. The currently retained failed-run cache contains 13 complete candidate archives and no `.part` files; those archives are reused only if their hashes match the next run's frozen closure. Passing closure evidence must prove that the initial cache audit partitions the entire frozen package set into reused or pending identities and that final `cached_before` annotations agree with that audit. A5 now adds per-package acquisition observability: every frozen identity has a current `pending`, `reused_verified`, `downloaded_verified`, `corrupt_removed`, or `failed` state plus verification/acquisition flags and last batch/attempt/mirror/failure-class fields. `progress_summary` records resolved, verified and pending package/byte totals and state totals, including a compact summary such as `667 resolved / 143 verified / 524 pending`. Each mirror attempt records requested/verified/pending filenames and byte totals, result and normalized failure class. The outer harness validates these records even on current failed runs, while historical pre-A5 failed evidence remains readable. A6 now adds structured repository-closure failure diagnosis while keeping the outer run stage `repository-closure`: failures identify `mirror-selection`, `repository-sync`, `resolution`, `acquisition`, `cache-verification`, or `local-validation`, with normalized causes such as timeout, TLS EOF, HTTP 404, hash mismatch, signature failure, DNS/connection failure, resolution mismatch, missing files, or generic process/runtime failure. Pacman acquisition stderr is captured for classification and replayed to the terminal, and the outer `build-run.json` failure reason includes the validated substage/cause. A7 now locks the final security boundary: synchronized repository DBs are copied into a separate read-only snapshot rather than the eventual `file://` repository; the local repository is not created until every frozen package is present, SHA-valid and ledger-verified; independent local-only re-resolution must then pass; cache ownership must be restored and the cache remounted read-only; and `buildiso` is launched only through an explicit in-process security gate whose evidence is also required by the outer ledger. Regression coverage proves mirror failover, failed-batch resume, cache reuse/corrupt cleanup, frozen-DB immutability, accurate partial evidence, and that one unresolved package prevents the buildiso runner from being called. The next required proof is empirical: exercise A0-A7 in a canonical native run. The tracked `memtest86+` correction remains native-unproven because this run never reached `buildiso`. No candidate ISO has been produced or accepted yet.
